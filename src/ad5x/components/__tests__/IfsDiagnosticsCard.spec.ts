...
describe('IfsDiagnosticsCard', () => {
  ...
  it('renders provider diagnostics without exposing control actions', () => {
    const wrapper = shallowMount(IfsDiagnosticsCard, { propsData: { diagnostics, stateCode: 5, activeSlot: 2, filamentAtToolhead: true } })
    ...
    expect(vm.rows.find((row: any) => row.test === 'diag-need-insert').value).toBe('да')
    expect(wrapper.text()).toContain('read-only')
    expect(wrapper.findAll('button').length).toBe(0)
  })

  it('keeps raw F13 channel separate from active slot and runtime slot', () => {
    const wrapper = shallowMount(IfsDiagnosticsCard, { propsData: { diagnostics, stateCode: 5, activeSlot: 2, filamentAtToolhead: null } })
    ...
    expect(rows.find((row: any) => row.test === 'diag-toolhead').value).toBe('неизвестно')
  })
})
